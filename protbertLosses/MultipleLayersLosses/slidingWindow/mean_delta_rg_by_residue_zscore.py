#!/usr/bin/env python3
"""
mean_delta_rg_by_residue_zscore.py

Compute mean *normalized* ΔRg per amino acid, pooled across:

  - all model_* directories under --results_root
  - selected window sizes (e.g. 3–5)

For each sequence and window:
  - z-score the effects within that sequence (per window size)
  - optionally flip sign (w = -z) if you want "compaction positive"
  - assign the normalized effect to every residue in the fragment

Then average per amino acid.
"""
...
def parse_args():
    ...
    ap.add_argument(
        "--flip_sign",
        action="store_true",
        help="If set, use w = -z so that compaction-promoting windows are positive.",
    )
    ap.add_argument(
        "--out_csv",
        default="mean_delta_rg_by_residue_zscore.csv",
        help="Output CSV filename.",
    )
    ap.add_argument(
        "--out_png",
        default="mean_delta_rg_by_residue_zscore.png",
        help="Output PNG barplot filename.",
    )
    return ap.parse_args()


def zscore_vec(x, eps=1e-12):
...
def main():
    ...
    for mdir in model_dirs:
        ...
        for w in requested_windows:
            ...
            for eff_seq, frag_seq in zip(
                ...
                # z-score per sequence (for this window size)
                z = zscore_vec(eff_seq)
                if args.flip_sign:
                    z = -z  # now "compaction-positive" if you want

                for z_val, frag in zip(z, frag_seq):
                    frag = str(frag)
                    if not frag:
                        continue
                    for aa in frag:
    ...
    df = pd.DataFrame({"AA": aas, "count": counts, "mean_z_effect": mean_vals})
    df.to_csv(args.out_csv, index=False)
    print(f"\n[saved] Mean normalized effect per residue → {args.out_csv}")

    plt.figure(figsize=(8, 4))
    plt.bar(df["AA"], df["mean_z_effect"])
    plt.axhline(0.0, color="black", linewidth=0.8)
    plt.xlabel("Amino acid")
    plt.ylabel("Mean normalized effect (z-score)")
    plt.title(
        f"Mean normalized ΔRg per residue\n"
