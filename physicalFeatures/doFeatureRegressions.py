...
with open('protein_features.csv', newline='') as f:
    reader = csv.reader(f)
    counter = 0
    for row in reader:
        if counter > 0:
...
features2 = features1[:, :15]
features3 = []
with open('protein_features2.csv', newline='') as f:
    reader = csv.reader(f)
    counter = 0
    for row in reader:
        if counter > 0:
        ...
        counter += 1
features3 = np.array(features1, dtype='float')
labels = []
inlierLabels = []
...
inlierFeatures2 = features2[inl]
inlierFeatures3 = features3[inl]
...
with open('pfeatureLosses.csv', 'w', newline='') as f:
    ...
    for ls in labelSplits:
        ...
        losses1 = evaluate_models_rmse(features1, label)
        lossesi1 = evaluate_models_rmse(inlierFeatures1, inlierLabel)
        ...
        losses2 = evaluate_models_rmse(features2, label)
        lossesi2 = evaluate_models_rmse(inlierFeatures2, inlierLabel)
        ...
        losses3 = evaluate_models_rmse(features3, label)
        lossesi3 = evaluate_models_rmse(inlierFeatures3, inlierLabel)
